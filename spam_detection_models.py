...
def predict_from_saved_bert(text):
    model_name = './models/bert-classifier.keras'
    # text = '''the great achievement of the catholic church lay in harmonizing , civilizing the deepest impulses of ordinary , ignorant people .
    # if you think your teacher is tough , wait until you get a boss . he doesn ' t have tenure .'''

    with keras.utils.custom_object_scope({'BertClassifier':BertClassifier, 'loss':keras.losses.SparseCategoricalCrossentropy(from_logits=True)}):
        model = keras.models.load_model(model_name)

    prediction = model.predict([text])

    if argmax(prediction, axis=-1).numpy()==0:
        print('Spam')
    else:
        print('Ham')
...
parser.add_argument('-p', '--predict', dest='text', action='store', type=str, help='predict spam/ham')
parser.add_argument('-t', required=False, action='store_true', help='train models')
...
if args['text'] != None:
    predict_from_saved_bert(args['text'])